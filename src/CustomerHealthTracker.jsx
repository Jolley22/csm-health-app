...
const CustomerHealthTracker = ({ session, onSignOut }) => {
  ...
  if (surveyMode) {
    ...
    const currentResponses = surveyResponses[currentCustomer.id] || {};
          ...
          <div className="bg-white rounded-lg shadow-md p-6 mb-6">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-gray-600">CSM: {surveyCSM}</p>
              </div>
              <button
                onClick={exitSurvey}
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-sm text-gray-600">
                <span>Progress: {completedCount} of {surveyCustomers.length} completed</span>
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            <div className="mb-6">
              <div className="flex items-center justify-between mb-2">
                <h2 className="text-xl font-bold text-gray-900">{currentCustomer.name}</h2>
                <span className="text-sm text-gray-500">
                  Customer {currentCustomerIndex + 1} of {surveyCustomers.length}
                </span>
              </div>
              <div className="flex gap-4 text-sm text-gray-600">
                {currentCustomer.segment && <span>Segment: {currentCustomer.segment}</span>}
              </div>
            </div>
            ...
            <div className="space-y-6">
              {Object.keys(metricLabels).map(metric => {
                const isOptional = optionalMetrics.includes(metric);
                const guidelines = metricGuidelines[metric];
                return (
                  <div key={metric} className="border border-gray-200 rounded-lg p-4 bg-gray-50">
                    <label className="block mb-3">
                      ...
                      <p className="text-xs text-gray-600 mt-1">{metricDescriptions[metric]}</p>
                    </label>
              ...
              <div>
                ...
                <textarea
                  value={currentResponses.notes || ''}
                  onChange={(e) => handleSurveyNotesChange(currentCustomer.id, e.target.value)}
                  rows="3"
            ...
            <div className="flex items-center justify-between mt-8 pt-6 border-t border-gray-200">
              ...
              {currentCustomerIndex === surveyCustomers.length - 1 ? (
                <button
